...
use quadmod
implicit none
real(8),intent(in ) :: fc(0:nfx,0:nfy)
real(8),intent(out) :: gp(ngx,ngy)
integer             :: j

! transpose, reorder and compute complex to real coefficients

gp(:,:) = 0.0 
gp(1,1:nfx+1) = fc(:,0)

gp(3:nfy+2:2,1) = fc(0,1:nky)
gp(4:nfy+2:2,1) = fc(1,1:nky)

do j = 2 , nfx , 2
   gp(3:nfy+2:2,j+1) = 0.5 * (fc(j  ,nfy:nky+1:-1) + fc(j  ,  1:nky     ))
   gp(4:nfy+2:2,j+2) = 0.5 * (fc(j  ,nfy:nky+1:-1) - fc(j  ,  1:nky     ))
   gp(4:nfy+2:2,j+1) = 0.5 * (fc(j+1,  1:nky     ) - fc(j+1,nfy:nky+1:-1))
   gp(3:nfy+2:2,j+2) = 0.5 * (fc(j+1,  1:nky     ) + fc(j+1,nfy:nky+1:-1))
enddo

call fc2gp(gp,ngy,nfx+2)
gp(:,:) = transpose(gp)
...
integer :: i,j,k,l,m,kstop,iink,jbase,kb,kc,kd
integer :: i0,i1,i2,i3,i4,i5,i6,i7
...
real(8) :: c1,c2,c3
real(8) :: s1,s2,s3
real(8) :: a0p2,a0m2,a1p3,a1m3
real(8) :: a4p6,a4m6,a5p7,a5m7
real(8) :: a0p2m1p3,a4m6m5m7
...
real(8) :: a1p5,a1m5
real(8) :: a2p6,a2m6
real(8) :: a0p7p3,a0p7m3
real(8) :: a0m7p4,a0m7m4
real(8) :: a1m5p2p6,a1m5m2p6
integer :: i0,i1,i2,i3,i4,i5,i6,i7
